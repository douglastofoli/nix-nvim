...
let
  ...
  collectKeymapsFromOne =
    cfg:
    if cfg == null || !(isAttrs cfg) then
      [ ]
    else
      let
  ...
  # Flatten plugin tree (same logic as plugins-aggregate.nix)
  ...
  collectWhichKeyGroupsFromOne =
    cfg: if cfg == null || !(isAttrs cfg) then [ ] else (cfg.keymaps or { }).whichKeyGroups or [ ];
    ...
    let
      optEntry =
        ...
        if builtins.typeOf v == "bool" then
          ...
          "";
      noremapVal = if (opts.noremap or true) then "true" else "false";
      parts = [ "noremap = ${noremapVal}" ];
      parts' = parts ++ (if opts ? desc then [ "desc = \"${escapeLuaStr opts.desc}\"" ] else [ ]);
      silentVal = if opts.silent then "true" else "false";
      parts'' = parts' ++ (if opts ? silent then [ "silent = ${silentVal}" ] else [ ]);
  ...
  mkKeymapLua =
    ...
    let
      ...
      rhsArg =
        if (km.rhsLua or null) != null && km.rhsLua != "" then
          km.rhsLua
        else
          "\"" + escapeLuaStr km.rhs + "\"";
      optsStr = optsToLua (km.opts // { noremap = km.noremap; });
    in
    "vim.keymap.set(${modeStr}, ${lhsStr}, ${rhsArg}, ${optsStr})";

  whichKeyGroupsLua =
    if allWhichKeyGroups == [ ] then
      ""
    else
      let
        groupLines = map (
